          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => {
              ...
              const shouldShowDropdowns = 
                ...
                item.name?.includes('ดอกไม้แห้ง') ||
                item.category?.includes('ดอกไม้แห้ง') || 
                item.category?.toLowerCase().includes('dried') || 
                item.isCustom ||
                item.isDriedFlower;

              const val1 = customSelections[`${itemId}-1`] || '';
              const val2 = customSelections[`${itemId}-2`] || '';
              const val3 = customSelections[`${itemId}-3`] || '';
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        {item.customOptions && item.customOptions.length > 0 ? (
                              ...
                              <div className="beauty-select-wrap">
                                <select 
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setCustomSelections(prev => {
                                      const updated = { ...prev, [`${itemId}-1`]: val };
                                      if (!val) {
                                        delete updated[`${itemId}-2`];
                                        delete updated[`${itemId}-3`];
                                      }
                                      return updated;
                                    });
                                  }}
                                >
                                  <option value="">รูปแบบดอกที่ 1</option>
                                  <option value="rose">ดอกกุหลาบ</option>
                                  <option value="tulip">ดอกทิวลิป</option>
                                </select>
                              </div>
                                ...
                                <div className="beauty-select-wrap">
                                  <select 
                                    ...
                                    onChange={(e) => {
                                      const val = e.target.value;
                                      setCustomSelections(prev => {
                                        const updated = { ...prev, [`${itemId}-2`]: val };
                                        if (!val) {
                                          delete updated[`${itemId}-3`];
                                        }
                                        return updated;
                                  ...
                                  >
                                    <option value="">รูปแบบดอกที่ 2</option>
                                    <option value="statice">ดอกสแตติส</option>
                                    <option value="gypso">ดอกยิปโซ</option>
                                  </select>
                                </div>
                              )}
                              ...
                              {val1 && val2 && (
                                <div className="beauty-select-wrap">
                                  <select 
                                    value={val3}
                                    className="beauty-select"
                                    onChange={(e) => setCustomSelections({ ...customSelections, [`${itemId}-3`]: e.target.value })}
                                  >
                                    <option value="">รูปแบบดอกที่ 3</option>
                                    <option value="ribbon">ริบบิ้นตกแต่ง</option>
                                    <option value="paper">กระดาษห่อช่อ</option>
                                  </select>
                                </div>
                              )}
                            </div>
                          ...
                          <span style={{ fontSize: '0.85rem', color: '#888' }}>
                            {item.description || 'Size L ช่อดอกสำเร็จรูป'}
                          </span>
                        )}
                      </div>
                    </div>
